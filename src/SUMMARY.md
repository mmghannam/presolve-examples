# Summary

[Preface](intro.md)

# Foundations

- [Notation](notation.md)

# Reductions for individual constraints

- [Overview](03-single-row/README.md)
- [3.1 Model cleanup and redundant constraints](03-single-row/3.1-model-cleanup.md)
- [3.2 Bound strengthening](03-single-row/3.2-bound-strengthening.md)
- [3.3 Coefficient strengthening](03-single-row/3.3-coefficient-strengthening.md)
- [3.4 Chvátal–Gomory strengthening](03-single-row/3.4-chvatal-gomory.md)
- [3.5 Euclidean and modular inverse reduction](03-single-row/3.5-euclidean-modular-inverse.md)
- [3.6 Simple probing on a single equation]()
- [3.7 Special ordered set reductions]()

# Reductions for individual variables

- [Overview](04-single-column/README.md)
- [4.1 Removal of fixed variables](04-single-column/4.1-fixed-variables.md)
- [4.2 Rounding bounds of integer variables](04-single-column/4.2-rounding-integer-bounds.md)
- [4.3 Strengthen semi-continuous and semi-integer bounds]()
- [4.4 Dual fixing, substitution, and bound strengthening]()
- [4.5 Substitute implied free variables](04-single-column/4.5-implied-free-substitution.md)

# Reductions on multiple constraints

- [Overview](05-multi-row/README.md)
- [5.1 Redundancy detection]()
- [5.2 Parallel and nearly parallel rows]()
- [5.3 Non-zero cancellation]()
- [5.4 Multi-row bound and coefficient strengthening]()
- [5.5 Clique merging]()

# Reductions on multiple variables

- [Overview](06-multi-column/README.md)
- [6.1 Multi-column dual fixing]()
- [6.2 Fix redundant penalty variables]()
- [6.3 Parallel columns]()
- [6.4 Dominated columns]()

# Full-problem reductions

- [Overview](07-full-problem/README.md)
- [7.1 Aggregate pairs of symmetric variables]()
- [7.2 Probing]()
- [7.3 Disconnected components]()
- [7.4 Biconnected components]()
- [7.5 Exploiting complementary slackness]()
- [7.6 Implied integer detection]()

# Node presolve

- [Overview](08-node-presolve/README.md)
- [8.1 Bound strengthening at nodes]()
- [8.2 Orbital fixing]()
- [8.3 Conflict propagation]()
- [8.4 Global probing]()

---

# Unreviewed drafts

- [About these drafts](drafts/README.md)
- [3.6 Simple probing on a single equation](03-single-row/3.6-simple-probing.md)
- [3.7 Special ordered set reductions](03-single-row/3.7-sos-reductions.md)
- [4.3 Strengthen semi-continuous and semi-integer bounds](04-single-column/4.3-semi-continuous-bounds.md)
- [4.4 Dual fixing, substitution, and bound strengthening](04-single-column/4.4-dual-fixing.md)
- [5.1 Redundancy detection](05-multi-row/5.1-redundancy-detection.md)
- [5.2 Parallel and nearly parallel rows](05-multi-row/5.2-parallel-rows.md)
- [5.3 Non-zero cancellation](05-multi-row/5.3-non-zero-cancellation.md)
- [5.4 Multi-row bound and coefficient strengthening](05-multi-row/5.4-multirow-strengthening.md)
- [5.5 Clique merging](05-multi-row/5.5-clique-merging.md)
- [6.1 Multi-column dual fixing](06-multi-column/6.1-multicol-dual-fixing.md)
- [6.2 Fix redundant penalty variables](06-multi-column/6.2-redundant-penalty.md)
- [6.3 Parallel columns](06-multi-column/6.3-parallel-columns.md)
- [6.4 Dominated columns](06-multi-column/6.4-dominated-columns.md)
- [7.1 Aggregate pairs of symmetric variables](07-full-problem/7.1-symmetric-aggregation.md)
- [7.2 Probing](07-full-problem/7.2-probing.md)
- [7.3 Disconnected components](07-full-problem/7.3-disconnected-components.md)
- [7.4 Biconnected components](07-full-problem/7.4-biconnected-components.md)
- [7.5 Exploiting complementary slackness](07-full-problem/7.5-complementary-slackness.md)
- [7.6 Implied integer detection](07-full-problem/7.6-implied-integer-detection.md)
- [8.1 Bound strengthening at nodes](08-node-presolve/8.1-node-bound-strengthening.md)
- [8.2 Orbital fixing](08-node-presolve/8.2-orbital-fixing.md)
- [8.3 Conflict propagation](08-node-presolve/8.3-conflict-propagation.md)
- [8.4 Global probing](08-node-presolve/8.4-global-probing.md)
